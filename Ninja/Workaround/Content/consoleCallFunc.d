 ...
 */
func void Patch_Workaround_CC_CallInit() {
    CC_Register(Patch_Workaround_CC_Call, "call ", "Call any daedalus or external function");
};
 ...
 */
func int Patch_Workaround_CC_Call_PushSymbol(var int symbID) {
 ...
 */
func string Patch_Workaround_CC_Call(var string query) {
        ...
        repeat(i, numParams);
            ...
            if (argIsStr) {
                ...
                if (argIsFloat) {
                    ...
                    // Push symbol
                    if (!Patch_Workaround_CC_Call_PushSymbol(argIntI)) {
                ...
                // Push symbol
                if (!Patch_Workaround_CC_Call_PushSymbol(aSymbID)) {
                    return ConcatStrings(ConcatStrings(errStr1, "Don't know how to push type "), pTypeS);
                };
...
func int Patch_Workaround_verifySymbol(var string name, var int type) {
...
func string setS(var string name, var string val) {
    var int symPtr; symPtr = Patch_Workaround_verifySymbol(name, zPAR_TYPE_STRING);
    if (symPtr) {
        var zCPar_Symbol sym; sym = _^(symPtr);
...
func string setI(var string name, var int val) {
    var int symPtr; symPtr = Patch_Workaround_verifySymbol(name, zPAR_TYPE_INT);
    if (symPtr) {
        var zCPar_Symbol sym; sym = _^(symPtr);
        sym.content = val;
        return "Done";
...
func string setF(var string name, var float val) {
    var int symPtr; symPtr = Patch_Workaround_verifySymbol(name, zPAR_TYPE_FLOAT);
    if (symPtr) {
        var zCPar_Symbol sym; sym = _^(symPtr);
